...
export default function FulfillmentPage() {
  ...
  const fetchOrders = useCallback(async ({ force = false } = {}) => {
    ...
    try {
      ...
      setAffiliateCodesFromDb(data.affiliate_codes || [])
  ...
  const affiliateOptions = Array.from(new Set([...affiliateCodesFromDb, ...orders.map(o => o.affiliate_code).filter(Boolean)])).sort(); const filtered = orders.filter(o => { if (affFilter && (o.affiliate_code || '') !== affFilter) return false;
