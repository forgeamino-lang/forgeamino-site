...
.from('affiliates')
.select('code, name, discount_pct, discount_to_cost, discount_to_fixed_price, product_prices, email_whitelist, active')
.ilike('code', code)
.eq('active', true)
.maybeSingle()
return data
...
// Fixed-price discount (FAMILY): discount = qty * (retail - fixed_price) per line.
// Items not in the product_prices map contribute $0 (no discount on that line).
let total_discount = 0
const per_line = []
for (const li of (line_items || [])) {
const cat = li.slug ? getProductBySlug(li.slug) : null
...
// For cost-based and fixed-price codes, sGET alone isn't sufficient — client should POST with cart + email.
...
// Fixed-price discount path (FAMILY)
