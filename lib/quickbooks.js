...
export async function getAccessToken() {
      ...
      if (data.refresh_token && data.refresh_token !== refreshToken) {
              persistRefreshToken(data.refresh_token).catch(e => {
                  // Log to Vercel (short retention) AND alert via Sentry so
                  // a silent rotation failure doesn't gradually cook QBO auth
                  // the way it did on 2026-04-19. Tagged for easy filtering.
