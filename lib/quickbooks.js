...
// Module-level cache of the current QBO refresh token.
// Seeded from env at cold start and updated in-memory whenever Intuit rotates
// it. This is the source of truth for warm lambdas - process.env is only read
// once, at module init. Before this fix, each getAccessToken call re-read
// process.env, which stayed stale after a rotation until the lambda restarted.
let currentRefreshToken = process.env.QBO_REFRESH_TOKEN

// Serialize concurrent refreshes on a single warm lambda. Without this, two
// in-flight orders can both trigger a refresh, both rotate the token at Intuit,
// and one of them ends up holding an already-superseded token - producing
// random invalid_grant failures under load.
let inflightRefresh = null

export async function getAccessToken() {
      if (!currentRefreshToken) throw new Error('QBO_REFRESH_TOKEN not set')
      ...
      inflightRefresh = (async () => {
              try {
                      ...
                      if (data.refresh_token && data.refresh_token !== currentRefreshToken) {
                              // Update in-memory FIRST. Intuit has already invalidated
                              // the old token; the new one is what is real from now on.
                              // Doing this before the Vercel write means warm lambdas
                              // keep working even if persistRefreshToken throws.
                              currentRefreshToken = data.refresh_token
                              // AWAIT the Vercel write. Fire-and-forget here is what
                              // caused the silent-loss incidents on 2026-04-19,
                              // 2026-04-21, and 2026-04-23: the lambda returned and
                              // got frozen before the Vercel API call finished,
                              // dropping the new token. On next cold start,
                              // process.env still held the now-invalid old token,
                              // and every new order blew up with invalid_grant until
                              // a manual re-auth.
                              try {
