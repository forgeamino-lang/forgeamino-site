...
export async function createSalesReceipt(accessToken, realmId, { customer, order }) {
  ...
  const response = await fetch(
    `${BASE_URL}/v3/company/${realmId}/salesreceipt?minorversion=65`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  const data = await response.json()
  if (!response.ok) throw new Error(`Failed to create QBO sales receipt: ${JSON.stringify(data)}`)
