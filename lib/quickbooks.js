...
export async function findOrCreateCustomer(accessToken, realmId, { name, email, phone, address }) {
  ...
  // Split sales tax into QBO's tax liability account instead of a line item.
  // Manual override: GlobalTaxCalculation='TaxExcluded' + TxnTaxDetail.TotalTax tells QBO not to
  // recompute tax and to post the pre-calculated amount to the sales tax payable account.
  if (order.tax_amount && order.tax_amount > 0) {
    payload.TxnTaxDetail = {
      TotalTax: parseFloat(parseFloat(order.tax_amount).toFixed(2)),
    }
