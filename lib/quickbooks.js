...
// ────────────────────────────────────────────────────────────────────────
// Sep 1 2026 duplicate-order incident: Intuit's API was hanging/timing
// out, and (a) the whole checkout request awaits this module's work
// before responding, so a hung QBO call held the customer's browser open
// until they gave up and re-submitted, creating duplicate orders; and
// (b) a couple of the QBO calls parse the response as JSON before
// checking whether it *is* JSON, so a gateway timeout that returns a
// plain-text body (e.g. "stream timeout") threw an uncaught SyntaxError
// instead of failing gracefully. qboFetch and qboJson below fix both:
// every QBO call now has a hard timeout, and every response is parsed
// safely regardless of what Intuit actually sent back.
// ────────────────────────────────────────────────────────────────────────
const QBO_TIMEOUT_MS = 8000

async function qboFetch(url, options = {}) {
  return fetch(url, { ...options, signal: AbortSignal.timeout(QBO_TIMEOUT_MS) })
}

async function qboJson(response) {
  const text = await response.text()
  try {
    return text ? JSON.parse(text) : {}
  } catch (e) {
    console.error('QBO response was not valid JSON:', { status: response.status, bodyPreview: text.slice(0, 200) })
    return { __parseError: true, __rawBody: text.slice(0, 500), __status: response.status }
  }
}
...
export async function exchangeCodeForTokens(code) {
      ...
      const response = await qboFetch(TOKEN_URL, {
              method: 'POST',
              headers: {
...
export async function getAccessToken() {
  ...
  inflightRefresh = (async () => {
    try {
      ...
      const data = await qboJson(response)
...
export async function findOrCreateCustomer(accessToken, realmId, { name, email, phone, address }) {
  ...
  const payload = {
      ...
      const createData = await qboJson(createRes)
...
// Checks whether a given DocNumber already exists on any invoice in QBO.
// Used by generateOrderNumber() (lib/supabase.js) as a defense-in-depth
// check before an order number is finalized -- Supabase's own uniqueness
// constraint only catches collisions with orders placed through this site;
// it can't see historical or manually-created QBO invoices (see Aug 2026
// incident notes in lib/supabase.js). This does a single targeted query
// (not a full invoice list), so it stays fast enough to run inline during
// order creation. Throws on request failure so the caller (which wraps
// this in try/catch) can fall back gracefully instead of silently trusting
// a bad result.
export async function checkInvoiceDocNumberExists(accessToken, realmId, docNumber) {
  const escaped = String(docNumber).replace(/'/g, "\\'")
  const query = `SELECT Id FROM Invoice WHERE DocNumber = '${escaped}'`
  const url = `${BASE_URL}/v3/company/${realmId}/query?query=${encodeURIComponent(query)}&minorversion=65`
  const resp = await qboFetch(url, {
    cache: 'no-store',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      Accept: 'application/json',
  ...
  const data = await qboJson(resp)
  if (!resp.ok) {
    throw new Error(`QBO DocNumber check failed: ${JSON.stringify(data).slice(0, 200)}`)
  }
  return (data?.QueryResponse?.Invoice || []).length > 0
}
...
export async function fetchQboItems(accessToken, realmId) {
  ...
  for (const it of data?.QueryResponse?.Item || []) {
    if (it.Active !== false) byName.set(it.Name.toLowerCase(), it)
  }
  return byName
}
...
async function createInvoice(accessToken, realmId, { customer, order }) {
      ...
      const lines = (order.line_items || []).map((item, idx) => {
        ...
        const matched = qboItemsByName.get(invName.toLowerCase())
        ...
        return {
          ...
          Description: matched?.Description || invName,
          SalesItemLineDetail: {
            ...(ref ? { ItemRef: ref } : {}),
            Qty: item.quantity,
      ...
      // Append a Discount line BEFORE shipping when the order carries a
      // customer-facing discount (e.g. FRIENDS = 10% off product subtotal).
      const discountAmount = Number(order.discount_amount ?? order.shipping_address?.discount_amount ?? 0)
      if (discountAmount > 0) {
        lines.push({
          Id: String(lines.length + 1),
          Amount: parseFloat(discountAmount.toFixed(2)),
          DetailType: 'DiscountLineDetail',
          Description: `Discount (${order.promo_code || order.shipping_address?.promo_code || order.affiliate_code || 'code'})`,
      ...
      const noteParts = [`Order #${order.order_number}`, `Payment: ${order.payment_method || 'card'}`]
      if (order.affiliate_code) noteParts.push(`Affiliate: ${order.affiliate_code}`)
      if (unmatchedNames.length) noteParts.push(`Unmatched items (fell back to Services): ${unmatchedNames.join(', ')}`)
      const privateNote = noteParts.join(' | ')
            ...
            const payload = {
                    ...
                    // Print the customer's phone number directly on the invoice for the fulfillment team
                    // (mirrors the CustomerMemo precedent below for surfacing customer_email on the PDF).
                    CustomField: [
                    	{ DefinitionId: '1000000001', Name: 'Phone', Type: 'StringType', StringValue: order.customer_phone || '' },
                    ],
                    ShipAddr: {
                    ...
                    // Shipping team requested the customer email be visible directly on
                    // the invoice itself (BillEmail alone only drives QBO's send-to
                    // address and doesn't print on the PDF). CustomerMemo prints on
                    // every invoice template, so it's the reliable place for this.
                    CustomerMemo: { value: `Forge Amino Order #${order.order_number} | Email: ${order.customer_email || 'N/A'}` },
...
export async function fetchInvoicePdf(accessToken, realmId, invoiceId) {
