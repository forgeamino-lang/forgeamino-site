...
// EMAIL 1: Order received &mdash; sent immediately to customer
export async function sendOrderConfirmationEmail(order) {
  ...
  const html = emailWrapper(`
    ...
    <div style="background:#fff3cd;border:2px solid #ffc107;border-radius:6px;padding:20px;margin-bottom:24px;">
      <h3 style="margin:0 0 12px;color:#0d1b2a;font-size:16px;">&#9888;&#xFE0F; Payment Instructions &mdash; Read Carefully</h3>
      ${paymentInstructions}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
      ...
      <tr>
        <td colspan="2" style="padding:4px 8px;color:#555;font-size:14px;">
          Tax (${order.shipping_address.city ? order.shipping_address.city + ', ' : ''}${order.shipping_address.state} &mdash; ${((order.tax_rate ?? 0) * 100).toFixed(2).replace(/\.00$/, '')}% combined)
        </td>
  ...
  return resend.emails.send({
    ...
    subject: `Order ${order.order_number} Received &mdash; Payment Instructions Inside`,
    html,
  })
}

// EMAIL 2: Payment confirmed &mdash; sent when admin marks order as paid
export async function sendPaymentConfirmedEmail(order) {
  const html = emailWrapper(`
    <h2 style="color:#0d1b2a;margin:0 0 8px;">Payment Confirmed â</h2>
    <p style="color:#555;margin:0 0 24px;">Great news &mdash; we've received your payment for order <strong>${order.order_number}</strong>. Your order is now being prepared for shipment.</p>

    <div style="background:#d4edda;border:2px solid #28a745;border-radius:6px;padding:16px;margin-bottom:24px;text-align:center;">
      <p style="margin:0;font-size:14px;color:#155724;font-weight:bold;">â Payment Received &mdash; Order Processing</p>
    </div>
  ...
  return resend.emails.send({
    ...
    reply_to: REPLY_TO,
    to: order.customer_email,
    subject: `Order ${order.order_number} &mdash; Payment Confirmed`,
    html,
  })
}

// EMAIL 3: Shipped &mdash; sent when admin enters tracking and marks shipped
...
export async function sendAdminOrderAlert(order) {
  const html = emailWrapper(`
    ...
    <ul>${order.line_items.map(i => `<li>${i.name} x${i.quantity} &mdash; $${(i.price * i.quantity).toFixed(2)}</li>`).join('')}</ul>
  ...
  return resend.emails.send({
    ...
    reply_to: REPLY_TO,
    to: ADMIN_EMAIL,
    subject: `New Order ${order.order_number} &mdash; $${order.total.toFixed(2)} via ${order.payment_method}`,
    html,
  })
}
