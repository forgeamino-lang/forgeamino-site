...
// EMAIL 1: Order received â sent immediately to customer
export async function sendOrderConfirmationEmail(order) {
  ...
  const html = emailWrapper(`
    ...
    <div style="background:#fff3cd;border:2px solid #ffc107;border-radius:6px;padding:20px;margin-bottom:24px;">
      <h3 style="margin:0 0 12px;color:#0d1b2a;font-size:16px;">â ï¸ Payment Instructions â Read Carefully</h3>
      ${paymentInstructions}
      <div style="background:#dc3545;border-radius:4px;padding:12px;margin-top:16px;">
        <p style="margin:0;color:#fff;font-size:13px;font-weight:bold;text-align:center;">
          â You MUST write exactly "<strong>${VENMO_NOTE_REQUIRED}</strong>" in the comment/note field.<br>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
      ...
      <tr>
        <td colspan="2" style="padding:4px 8px;color:#555;font-size:14px;">
          Tax (${order.shipping_address.city ? order.shipping_address.city + ', ' : ''}${order.shipping_address.state} â ${((order.tax_rate ?? 0) * 100).toFixed(2).replace(/\.00$/, '')}% combined)
        </td>
  ...
  return resend.emails.send({
    ...
    subject: `Order ${order.order_number} Received â Payment Instructions Inside`,
    html,
  })
}

// EMAIL 2: Payment confirmed â sent when admin marks order as paid
export async function sendPaymentConfirmedEmail(order) {
  const html = emailWrapper(`
    <h2 style="color:#0d1b2a;margin:0 0 8px;">Payment Confirmed â</h2>
    <p style="color:#555;margin:0 0 24px;">Great news â we've received your payment for order <strong>${order.order_number}</strong>. Your order is now being prepared for shipment.</p>

    <div style="background:#d4edda;border:2px solid #28a745;border-radius:6px;padding:16px;margin-bottom:24px;text-align:center;">
      <p style="margin:0;font-size:14px;color:#155724;font-weight:bold;">â Payment Received â Order Processing</p>
    </div>
    ...
    <p style="color:#555;font-size:14px;">We'll send you another email with your tracking number as soon as your order ships. Typical processing time is 1â2 business days.</p>
  `)
  ...
  return resend.emails.send({
    ...
    reply_to: REPLY_TO,
    to: order.customer_email,
    subject: `Order ${order.order_number} â Payment Confirmed`,
    html,
  })
}

// EMAIL 3: Shipped â sent when admin enters tracking and marks shipped
export async function sendShippedEmail(order) {
  const html = emailWrapper(`
    <h2 style="color:#0d1b2a;margin:0 0 8px;">Your Order Has Shipped! ð¦</h2>
...
export async function sendAdminOrderAlert(order) {
  const html = emailWrapper(`
    <h2 style="color:#0d1b2a;margin:0 0 8px;">ð New Order: ${order.order_number}</h2>
    <p><strong>Customer:</strong> ${order.customer_name} (${order.customer_email})</p>
    <p><strong>Total:</strong> $${order.total.toFixed(2)}${order.tax_amount > 0 ? ` (incl. $${order.tax_amount.toFixed(2)} tax)` : ''}</p>
    <p><strong>Payment Method:</strong> ${order.payment_method.toUpperCase()}</p>
    <p><strong>Items:</strong></p>
    <ul>${order.line_items.map(i => `<li>${i.name} x${i.quantity} â $${(i.price * i.quantity).toFixed(2)}</li>`).join('')}</ul>
    ...
    <p style="margin-top:24px;">
      <a href="${process.env.NEXT_PUBLIC_SITE_URL}/admin/orders/${order.id}"
         style="background:#0d1b2a;color:#fff;padding:12px 24px;border-radius:6px;text-decoration:none;font-weight:bold;">
        View Order in Admin
      </a>
    </p>
  `)
  ...
  return resend.emails.send({
    ...
    reply_to: REPLY_TO,
    to: ADMIN_EMAIL,
    subject: `New Order ${order.order_number} â $${order.total.toFixed(2)} via ${order.payment_method}`,
    html,
  })
}
