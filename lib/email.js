...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'swmrdude15@gmail.com'
const FROM_EMAIL = 'orders@forgeamino.com'
...
export async function sendOrderConfirmationEmail(order) {
  ...
  const html = emailWrapper(`
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
      ...
      <tr>
        <td colspan="2" style="padding:12px 8px 0;font-weight:bold;">Total</td>
        <td style="padding:12px 8px 0;text-align:right;font-weight:bold;font-size:18px;">$${order.total.toFixed(2)}</td>
      </tr>
    </table>
...
export async function sendAdminOrderAlert(order) {
  const html = emailWrapper(`
    <h2 style="color:#0d1b2a;margin:0 0 8px;">🛒 New Order: ${order.order_number}</h2>
    <p><strong>Customer:</strong> ${order.customer_name} (${order.customer_email})</p>
    <p><strong>Total:</strong> $${order.total.toFixed(2)}</p>
    <p><strong>Payment Method:</strong> ${order.payment_method.toUpperCase()}</p>
    <p><strong>Items:</strong></p>
    <ul>${order.line_items.map(i => `<li>${i.name} x${i.quantity} — $${(i.price * i.quantity).toFixed(2)}</li>`).join('')}</ul>
  ...
  return resend.emails.send({
    from: FROM_EMAIL,
    to: ADMIN_EMAIL,
    subject: `New Order ${order.order_number} — $${order.total.toFixed(2)} via ${order.payment_method}`,
    html,
  })
}
