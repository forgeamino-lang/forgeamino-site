...
const paymentInstructions = getPaymentInstructions(order.payment_method, order.total, order.ach_fee_amount ?? order.shipping_address?.ach_fee_amount ?? 0)
const html = emailWrapper(`
<h2 style="color:#0d1b2a;margin:0 0 8px;">Order Received!</h2>
<p style="color:#555;margin:0 0 12px;">Thanks ${order.customer_name.split(' ')[0]}, we've received your order.${order.payment_method === 'lut_ach' ? ' Your payment has been processed and we\'ll get it prepared for shipment.' : ' Complete your payment using the instructions below and we\'ll get it processed.'}</p>

<p style="color:#666;margin:0 0 24px;font-size:13px;">&#128231; This email is from <strong>orders@forgeamino.us</strong> &mdash; please add us to your contacts so future order updates always reach your inbox.</p>
...
<h3 style="margin:0 0 12px;color:#0d1b2a;font-size:16px;">${order.payment_method === 'lut_ach' ? '&#10003; Payment Submitted' : '&#9888;&#xFE0F; Payment Instructions — Read Carefully'}</h3>
${paymentInstructions}
...
${(order.tax_amount > 0 || Number(order.discount_amount ?? order.shipping_address?.discount_amount ?? 0) > 0) ? `
<tr>
<td colspan="2" style="padding:10px 8px 0;color:#555;font-size:14px;">Subtotal</td>
<td style="padding:10px 8px 0;text-align:right;color:#555;font-size:14px;">$${(order.subtotal_before_discount ?? order.shipping_address?.subtotal_before_discount ?? order.subtotal ?? order.total).toFixed(2)}</td>
</tr>
${Number(order.discount_amount ?? order.shipping_address?.discount_amount ?? 0) > 0 ? `
<tr>
<td colspan="2" style="padding:4px 8px;color:#0a7d23;font-size:14px;">Discount (${order.affiliate_code || 'code'} — ${(((order.discount_amount ?? order.shipping_address?.discount_amount) / (order.subtotal_before_discount ?? order.shipping_address?.subtotal_before_discount ?? 1)) * 100).toFixed(0)}% off)</td>
<td style="padding:4px 8px;text-align:right;color:#0a7d23;font-size:14px;">−$${Number(order.discount_amount ?? order.shipping_address?.discount_amount ?? 0).toFixed(2)}</td>
</tr>` : ''}
...
${(order.shipping_address?.shipping_method || order.shipping_method) ? `
<tr><td colspan="2" style="padding:4px 8px;color:#555;font-size:14px;">Shipping (${(order.shipping_method || order.shipping_address?.shipping_method) === 'local_delivery' ? 'Local Delivery' : 'FedEx 2-Day'})</td><td style="padding:4px 8px;text-align:right;color:#555;font-size:14px;">${Number(order.shipping_amount ?? order.shipping_address?.shipping_amount ?? 0) === 0 ? 'FREE' : '$' + Number(order.shipping_amount ?? order.shipping_address?.shipping_amount ?? 0).toFixed(2)}</td></tr>` : ''}
${Number(order.ach_fee_amount ?? order.shipping_address?.ach_fee_amount ?? 0) > 0 ? `
<tr><td colspan="2" style="padding:4px 8px;color:#555;font-size:14px;">ACH Processing Fee (3.95% + $0.30)</td><td style="padding:4px 8px;text-align:right;color:#555;font-size:14px;">$${Number(order.ach_fee_amount ?? order.shipping_address?.ach_fee_amount ?? 0).toFixed(2)}</td></tr>` : ''}
<tr>
...
<p style="margin:24px 0 0;color:#888;font-size:13px;">${order.payment_method === 'lut_ach' ? 'We\'ll send you a shipping update as soon as your order ships.' : 'Once we confirm your payment, we\'ll send you a shipping update.'} Questions? Reply to this email.</p>
`)
return resend.emails.send({
from: FROM_EMAIL,
reply_to: REPLY_TO,
to: order.customer_email,
subject: order.payment_method === 'lut_ach'
? `Order ${order.order_number} Received — Payment Confirmed`
: `Order ${order.order_number} Received — Payment Instructions Inside`,
html,
})
}
...
// EMAIL 4: Admin new order alert// EMAIL 5: Admin order-received alert — fires immediately on order save,
// independent of QBO. Ensures admin always knows an order arrived even when
// QuickBooks sync fails. The QBO-success email (sendAdminOrderAlert) still
// fires later if the invoice creates successfully and includes the PDF.
export async function sendOrderReceivedAlert(order) {
const orderNumber = order.order_number
const customerName = order.customer_name || 'Customer'
const customerEmail = order.customer_email || ''
const customerPhone = order.customer_phone || ''
const total = Number(order.total || 0).toFixed(2)
const subtotal = Number(order.subtotal_before_discount ?? order.shipping_address?.subtotal_before_discount ?? order.subtotal ?? order.shipping_address?.subtotal ?? 0).toFixed(2)
const discountAmount = Number(order.discount_amount ?? order.shipping_address?.discount_amount ?? 0).toFixed(2)
const taxAmount = Number(order.tax_amount ?? order.shipping_address?.tax_amount ?? 0).toFixed(2)
const shippingAmount = Number(order.shipping_amount ?? order.shipping_address?.shipping_amount ?? 0).toFixed(2)
const shippingMethod = order.shipping_method ?? order.shipping_address?.shipping_method ?? 'standard'
const paymentMethod = order.payment_method || 'unknown'
const addr = order.shipping_address || {}
const addressLine = [addr.line1, addr.line2, addr.city, addr.state, addr.zip].filter(Boolean).join(', ')
const itemRows = formatLineItems(order.line_items || [])

const html = emailWrapper(`
<h2 style="color:#0d1b2a;margin:0 0 12px;">New Order Received: ${orderNumber}</h2>
<p>An order has just been placed on forgeamino.com. QuickBooks sync is in progress — you'll receive a separate email with the invoice PDF once it completes.</p>
<p><strong>Customer:</strong> ${customerName}${customerEmail ? ` &lt;${customerEmail}&gt;` : ''}${customerPhone ? ` — ${customerPhone}` : ''}</p>
<p><strong>Shipping:</strong> ${addressLine || '(none)'} — ${shippingMethod}</p>
<p><strong>Payment method:</strong> ${paymentMethod}</p>
<table width="100%" cellpadding="0" cellspacing="0" style="margin-top:16px;border-top:2px solid #0d1b2a;border-bottom:2px solid #0d1b2a;">
<thead>
<tr><th align="left" style="padding:8px 0;">Item</th><th align="center" style="padding:8px 0;">Qty</th><th align="right" style="padding:8px 0;">Subtotal</th></tr>
</thead>
<tbody>${itemRows}</tbody>
</table>
<table width="100%" cellpadding="0" cellspacing="0" style="margin-top:12px;">
<tr><td align="right">Subtotal:</td><td align="right" width="100">$${subtotal}</td></tr>
${Number(discountAmount) > 0 ? `<tr><td align="right" style="color:#0a7d23;">Discount (${order.affiliate_code || 'code'}):</td><td align="right" width="100" style="color:#0a7d23;">−$${discountAmount}</td></tr>` : ''}
<tr><td align="right">Shipping (${shippingMethod}):</td><td align="right" width="100">$${shippingAmount}</td></tr>
<tr><td align="right">Tax:</td><td align="right" width="100">$${taxAmount}</td></tr>
<tr><td align="right" style="font-weight:bold;font-size:16px;">Total:</td><td align="right" width="100" style="font-weight:bold;font-size:16px;">$${total}</td></tr>
</table>
`)
...
subject: `New order ${orderNumber} — ${customerName} — $${total}`,
html,
})
}
...
function getPaymentInstructions(method, total, achFeeAmount = 0) {
if (method === 'lut_ach') {
return `
<p style="margin:0;font-size:15px;color:#333;">Your bank account payment for <strong>$${total.toFixed(2)}</strong> has been submitted via ACH through our payment partner, Lüt Turbo. No further action is needed on your end.</p>
${achFeeAmount > 0 ? `<p style="margin:8px 0 0;font-size:13px;color:#666;">This total includes a $${achFeeAmount.toFixed(2)} ACH processing fee (3.95% + $0.30) charged by our payment partner.</p>` : ''}
